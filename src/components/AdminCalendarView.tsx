...
export const AdminCalendarView: React.FC = () => {
  ...
  const [servicesPrices, setServicesPrices] = useState<{ [key: string]: number | { p: number; m: number; g: number } }>({});
    ...
    const unsubscribe = onValue(servicesRef, (snapshot) => {
      try {
        if (snapshot.exists()) {
          ...
          // Build prices mapping
          const prices: { [key: string]: number | { p: number; m: number; g: number } } = {};
          servicesList.forEach(service => {
            if (service.sizes) {
              prices[service.name] = service.sizes;
            } else if (service.price) {
              prices[service.name] = service.price;
            }
          });
          
          setServicesPrices(prices);
        }
      } catch (error) {
  ...
  const getTotalDuration = (services: string[]) => {
    ...
    // Find service durations from loaded services
    const durations = services.map(serviceName => {
      const service = services.find(s => s.name === serviceName);
      return service?.duration || 30; // Default to 30 minutes
    });
    
    // If any service is 60 minutes or more, return 60
    if (durations.some(duration => duration >= 60)) {
      return 60;
    }

    // Count services that have duration > 0
    const significantServices = durations.filter(duration => duration > 0);
    
    if (significantServices.length === 0) {
      return 0;
    }
    
    if (significantServices.length === 1) {
      return significantServices[0];
    }

    // Multiple services with duration > 0
    if (significantServices.length > 1) {
      return 60;
    }
  ...
  const needsEarlyArrival = selectedServices.some(service => 
    service.toLowerCase().includes('carbonoplastia') || service.toLowerCase().includes('taninoplastia')
  );
        ...
        <div className="bg-white p-3 sm:p-4 rounded-lg border border-[#E8D5C4]">
          <div className="flex justify-between items-center mb-3 sm:mb-4">
            ...
            <button
              ...
              className="bg-[#E3A872] text-white px-3 py-1.5 rounded-lg hover:bg-[#D89860] transition-colors duration-200 flex items-center text-sm"
            >
