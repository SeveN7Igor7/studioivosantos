...
export const ServiceSelection: React.FC<ServiceSelectionProps> = ({
  ...
  return (
    <div className="mt-6">
      <h3 className="text-lg font-medium text-gray-900 mb-4">Selecione os Serviços</h3>
      <div className="grid gap-3">
        {services.map((service) => (
          <button
            ...
            className={`
              flex items-start p-4 rounded-2xl border transition-all duration-200 w-full text-left
          ...
          >
            <div className={`
              flex-shrink-0 p-2 rounded-2xl mr-4
              ${selectedServices.includes(service.id) ? 'text-[#E3A872] bg-[#FDF8F3]' : 'text-gray-500 bg-gray-100'}
            `}>
              {service.icon}
            </div>
            <div className="flex-grow min-w-0">
              <h4 className="text-base font-medium text-gray-900 truncate">{service.name}</h4>
